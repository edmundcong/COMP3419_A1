...
List < PImage[] > displacement_list = new ArrayList < PImage[] > (); // array list of frames
...
// displacement vector
//PImage displacement_vector[][][] = new PImage[globalWidth/K][globalHeight/K][2]; // 0 -> x, 1 -> y
...
void draw() {
  if (playImages) {
    ...
    //PImage temp_overlay = displacement_list.get(currFrame);
    //image(temp_overlay, 0, 0);
    //System.out.println(temp_overlay.width);
        ...
        for (int i = 0; i < frameCounter; i++) {
          ...
          if (i + 1 < frameCounter) {
            ...
            for (int j = 0; j < totalGridBlocks; j++) {
              ...
              //B_i[j].save(sketchPath("") + "BG/"+nf(framenumber, 4) + ".tif"); // They say tiff is faster to save, but larger in disks
              float min_ssd = 0;
              ...
              //System.out.println("min_ssd: " + min_ssd);
              //displacement_x = (int) sqrt((min_x - curr_x)*(min_x - curr_x));
              //displacement_y = (int) sqrt((min_y - curr_y)*(min_y - curr_y));
              //displacement_y = min_y - curr_y;
              //displacement_x = min_x - curr_x;
              //System.out.println("displacement_x: " + displacement_x + " = " + min_x + " - " + curr_x);
              //System.out.println("displacement_y: " + displacement_y + " = " + min_y + " - " + curr_y);
              //image(j_transparent_overlay, 0, 0);
              //displacement_vector[j][min_ssd_index][0] = displacement_x;
              //displacement_vector[j][min_ssd_index][1] = displacement_y;
              //j_transparent_overlay.loadPixels();
              //j_transparent_overlay.pixels[displacement_y * 567 + displacement_x] = color(254);
              //j_transparent_overlay.updatePixels();
              // create a transparent image to overlay on our frame which has dots at displacement points
              //if (min_ssd_index != j) {
              //  long curr_unix_time = System.currentTimeMillis() / 1000;
              //  B_i[j].save(sketchPath("") + "BG/"+nf(min_ssd_index, 4) + curr_unix_time + "-j" + i + ".tif"); // They say tiff is faster to save, but larger in disks
              //  B_j[min_ssd_index].save(sketchPath("") + "BG/"+nf(min_ssd_index, 4) + curr_unix_time + "-min_ssd_index" + i + ".tif"); // They say tiff is faster to save, but larger in disks
            ...
            //frames.set(i + 1, B_j);
            //displacement_list.add(i, j_transparent_overlay);
          }
        }
        playImages = true;
      }
